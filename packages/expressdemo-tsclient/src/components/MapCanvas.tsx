...
import { GraphData, Node } from '../services/api';
...
export default function MapCanvas({ graphData, selectedPath, pathPoints }: MapCanvasProps) {
  ...
  const calculateScaledCoordinates = useCallback((nodes: Node[], width: number, height: number) => {
    if (!nodes?.length) return { scaledNodes: [], bounds: null };

    // Find bounds
    const bounds = nodes.reduce((acc: Bounds, node) => ({
  ...
  // Draw the graph
  const drawGraph = useCallback((ctx: CanvasRenderingContext2D, scaledNodes: ScaledNode[], lines: [number, number][] | undefined, path: number[] | null) => {
  ...
  useEffect(() => {
    ...
    if (graphData?.nodes) {
      ...
      drawGraph(ctx, scaledNodes, (graphData as any).line, selectedPath);
    }
